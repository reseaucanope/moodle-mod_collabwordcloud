...
require_once($CFG->dirroot.'/mod/collabwordcloud/wordcloud.php');
...
if (isset($params->cmid)) {
    $cm = get_coursemodule_from_id('collabwordcloud', $params->cmid);
    
    $wc = $DB->get_record('collabwordcloud', array('id' => $cm->instance));
    if ($wc === false) {
...
$PAGE->set_url('/mod/collabwordcloud/api.php');
...
if (isset($params->action) && isset($params->groupid) && isset($params->lastupdate)
    ...
    if (!has_capability('mod/collabwordcloud:submitword', $context)) {
    ...
    if (!has_capability('mod/collabwordcloud:manageword', $context)) {
        die('{"error":true,"msg":"Access denied"}');
    }
